...
static const QEVENT testQEvt[] = {{A_SIG, 0, 0}, {B_SIG, 0, 0}, {C_SIG, 0, 0}, {D_SIG, 0, 0}, {E_SIG, 0, 0}, {F_SIG, 0, 0}, {G_SIG, 0, 0}, {H_SIG, 0, 0}};

int main() {
	printf("Hierarchy state machine ...... \n");
	Initial(); 		// trigger initial transition
	for (;;) {
		char c;
		printf("\nSignal <-- ");
		c = getc(stdin);
...
QSTATE s11(QEVENT const *e) {
	switch (e->sig) {
		...
		case H_SIG: // internal transition with a guard
			if (bFoo) { // test the guard condition
				printf("s11-H;");
				bFoo = false ;
				return 0;
			}
...
QSTATE s21(QEVENT const *e) {
	switch (e->sig) {
		...
		case B_SIG: printf("s21-B;"); Q_TRAN(s211); return 0;
